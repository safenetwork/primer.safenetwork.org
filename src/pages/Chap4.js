...
const Chap4 = () => (
    ...
    <div className='chapter'>
        ...
        <p>Petar Maymounkov and David Mazi&egrave;res released the Kademlia distributed hash table in 2002. The idea is that nodes form a network overlay, and are identified with a different node identification system. So a node (in our case a Vault) could have an IP address of 96.251.182.97 while it uses 17846cb8a4b53c9e44c616d2415a15984283eee975a1dac8f488dd91d0aed1cd as a unique 256-bit address in XOR space.</p>
        ...
        <p>The nodes managing a Section will always try to reach consensus (agreement) amongst themselves on any Network event happening within that Disjoint Section. They also &lsquo;group sign&rsquo; messages that travel over the wider Network so that other nodes and/or Sections can cryptographically verify the validity of each message and action (such as groups forming and splitting). These group signatures are stored in &lsquo;SectionProofChains&rsquo; which are secured and held by members of that Section, and by neighbouring Sections, for the life of each session. This is known as Group Consensus.</p>
        ...
        <p>The closer a node is to a certain address space on the SAFE Network, the more information it has about data that is stored at that address. And logically, the further away a Vault is, the less information it has. The bigger the Network becomes, the more secure it will get because an individual Vault will have influence over a decreasing range of addresses.
                </p>
