...
const component = () => (
    ...
    <div className='chapter'>
        ...
        <p><em>Anonymization</em> - To retain anonymity, the identity of a Client connecting to the Network must be obfuscated from the nodes that comprise it. For this reason connections between Clients and Vaults in the SAFE Network always occur via a Vault the IP of which is in the config file. Any Vault can perform this role and rather than accepting the config file the user may prefer to connect via a known node of his or her choosing. Thereafter, the initial Vault drops the connection and cannot track the Client's activity (see Chapter 4).</p>
        ...
        <p>Someone with enough Vaults could of course bring the Network down (for example by suddenly turning off all their Vaults). But this gets harder very quickly as the Network grows larger. A combination of Node Ageing, churn, rules on joining and the splitting of Sections would make this massively more difficult (and very expensive) on a large network.</p>
