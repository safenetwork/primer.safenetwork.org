...
const Chap7 = () => (
    ...
    <div className='chapter'>
        ...
        <p>While such attacks have been successfully launched against some cryptocurrencies, Ethereum Classic being one example, Proof of Work has generally been effective in protecting against 51 percent attacks and at achieving BFT consensus across the network. But it is slow, massively energy intensive and it doesn&rsquo;t scale. Also, the consensus is probabilistic: after some time, you might be 99.9 percent sure that all nodes agree on network state, but never 100 percent.</p>
        <h3>Enter PARSEC</h3>
  
        <p>By contrast, the consensus algorithm used by the SAFE Network, PARSEC (Protocol for Asynchronous, Reliable, Secure and Efficient Consensus) is massively scalable, able to handle the millions of parallel transactions per second demanded by IoT-type applications, Byzantine fault tolerant, and provides 100 percent consensus that's mathematically provable. In time, all Elders (see Chapter 5) will agree on the same order of events, no matter what the physical distance between them, despite disparities between their resources, without requiring their &lsquo;votes&rsquo; to be fielded in any particular order (that&rsquo;s what the word &lsquo;asynchronous&rsquo; signifies). Because it is not blockchain-based, the SAFE Network can take advantage of some features developed for directed acyclic graphs (DAGs) such as gossip and concrete coins.</p>
        ...
        <p>Gossip graph - a record of events on the network and the order in which they happened. Each Elder holds its own gossip graph, and consensus is reached when their gossip graphs match.</p>
        <p>Valid block - an event that has been recorded by at least two thirds of the Elders.</p>
        <p>Concrete Coin - A protocol for forcing consensus in a random way in the event of a tie (created by Silvio Micali)</p>
        ...
        <p>At the time of writing, PARSEC represents the cutting edge of fully ABFT algorithms, with work ongoing to finalise its fully asynchronous nature by replacing the Concrete Coin with a common coin. A new white paper will be published in due course.</p>
