...
const Chap6 = () => (
    ...
    <div className='chapter'>
        <h2 id='ch6'>6. Everything's encrypted</h2>
        ...
        <p>As mentioned above, all data on the SAFE Network is encrypted (or held in containers that are encrypted). When a Client uploads a piece of data to the Network (for example an mp4 video) it is first broken into pieces with a maximum size of 1 MB and those pieces, or chunks, are then 'self-encrypted', a process patented by MaidSafe by which each chunk is encrypted using each chunk is encrypted using its own hash and the hashes of the two previous chunks in the same file. These encrypted chunks are then hashed again to arrive at the XOR address where the chunk will be stored. At the same time a Data Map is created on the Client device, which maps the chunk number to the XOR address of the chunk and the hash to decrypt it and its two successors, allowing the file to be recreated. A number of copies of each chunk are stored by Vaults in the Section to ensure redundancy.</p>
        ...
        <p>As shown, the SAFE Network uses several layers of encryption to protect a user's anonymity and privacy. Several extra layers are active when people use direct messaging or create a public profile. The Network is designed to be as 'zero-knowledge' as possible, to the extent that Farmers cannot possibly figure out what chunks from which file they are storing - even if it's their own. By utilizing multiple levels of encryption as well as obfuscating the identity of its users after the first hop, the SAFE Network provides a platform for applications that is both highly secure and anonymized by design.</p>
        <div className="What-does-that-mean">
        <h3>What does that mean?</h3>
        <p>Self-Encryption - Files uploaded to the SAFE Network are broken up into chunks. These chunks are then hashed, encrypted locally using the AES-256 encryption algorithm and the hashes of neighbouring chunks. Because they are encrypted and decrypted locally, there is no need for keys to leave your machine.</p>
        <p>Proxy Node - For each Client connection into the Network, there is an anonymizing Proxy Node which relays all data to and from destinations within the Network, but the proxy does not have the ability to read any of it (for those familiar with Tor, this function is akin to a 'guard node').</p>
